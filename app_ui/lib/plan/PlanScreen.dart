...
import 'package:app_ui/plan/DataFormatItem.dart';
...
class _PlanScreenState extends State<PlanScreen> {
  // DataFormat.dart 数据格式化
  List<DataFormat> listDataArr = [];

  void getDataList() {
    setState(() {
      // 将数据传到DataFormat.returnDataFormatJsonString()中去处理
      listDataArr = DataFormat.returnDataFormatJsonString("""{
        "list": [
               {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: new ListView.builder(
        itemCount: listDataArr.length,
        
        itemBuilder: (context, index) {
          var model = listDataArr[index];
          // return Text(model.name);

          // return DataFormatItem (model);

          // 添加跳转事件
          return InkWell(
            child: DataFormatItem (model),
            onTap: (){
